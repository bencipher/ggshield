...
def test_config_update_cmd(cli_fs_runner):
    """
    GIVEN a v1 config file
    WHEN `ggshield config update` is called
    THEN the file is converted to v2
    AND the v1 config file is kept as a backup
    """
    config_path = Path(".gitguardian.yaml")
    config_path.write_text(V1_CONFIG_CONTENT)

    result = cli_fs_runner.invoke(cli, ["config", "update"])
