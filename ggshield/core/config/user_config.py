...
class UserConfig:
    ...
    # `ggshield config update`, which would be odd.
    # Instead, we store them in this list and a result_callback() function displays
    # them when we quit. When `config update` runs, it clears this list, so nothing gets
    # displayed.
        ...
        try:
            if config_version == 2:
                ...
                self.deprecation_messages.append(
                    ...
                    " Run `ggshield config update` to update it."
                )
